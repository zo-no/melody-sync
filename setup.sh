...
print_header "MelodySync Setup (${OS_TYPE})"
...
if [[ "$MODE_CHOICE" == "2" ]]; then
    ...
    print_success "Mode: Localhost only (http://127.0.0.1:7760)"
else
    USE_CLOUDFLARE=true
...
if [[ "$USE_CLOUDFLARE" == true ]]; then
    ...
    # Get subdomain
    read -p "Enter subdomain for remote access (default: melodysync): " SUBDOMAIN
    SUBDOMAIN=${SUBDOMAIN:-melodysync}
...
if command -v claude &> /dev/null; then
    ...
    print_warning "No AI CLI tool found (claude / codex / cline). Install at least one before using MelodySync."
fi
...
EOF
    ...
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        ...
        cat > "$HOME/Library/LaunchAgents/com.melodysync.tunnel.plist" << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.melodysync.tunnel</string>
    <key>ProgramArguments</key>
    <array>
...
EOF
        print_success "Created: ~/Library/LaunchAgents/com.melodysync.tunnel.plist"
    fi
...
if [ -f ~/Library/LaunchAgents/com.melodysync.chat.plist ]; then
  launchctl load ~/Library/LaunchAgents/com.melodysync.chat.plist 2>/dev/null || echo "chat-server already loaded"
fi
...
EOF
    ...
    if [[ -f "$SYSTEMD_DIR/melodysync-proxy.service" ]]; then
        systemctl --user stop melodysync-proxy.service 2>/dev/null || true
        systemctl --user disable melodysync-proxy.service 2>/dev/null || true
        rm -f "$SYSTEMD_DIR/melodysync-proxy.service"
        print_success "Removed legacy melodysync-proxy.service"
    fi

    if [[ "$USE_CLOUDFLARE" == true ]]; then
        print_info "Creating cloudflared systemd service..."
        CLOUDFLARED_BIN=$(which cloudflared)
        cat > "$SYSTEMD_DIR/melodysync-tunnel.service" << EOF
[Unit]
Description=MelodySync Cloudflare Tunnel
After=network.target
...
WantedBy=default.target
EOF
        print_success "Created: ~/.config/systemd/user/melodysync-tunnel.service"
    fi
...
if systemctl --user list-unit-files melodysync-tunnel.service &>/dev/null; then
  systemctl --user start melodysync-tunnel.service
fi
echo "Services started!"
echo ""
echo "Check status with:"
...
systemctl --user stop melodysync-tunnel.service 2>/dev/null || true
echo "Services stopped!"
STOPEOF
    chmod +x "$SCRIPT_DIR/stop.sh"
    print_success "Created: stop.sh"
...
# MelodySync Remote Access Credentials
# Generated: $(date)
# OS: $OS_TYPE
...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    launchctl unload "$HOME/Library/LaunchAgents/com.authproxy.claude.plist" 2>/dev/null || true
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        launchctl unload "$HOME/Library/LaunchAgents/com.melodysync.tunnel.plist" 2>/dev/null || true
    fi
else
...
if [[ "$OS_TYPE" == "macos" ]]; then
    ...
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        launchctl load "$HOME/Library/LaunchAgents/com.melodysync.tunnel.plist"
    fi
    sleep 3
    ...
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        CLOUDFLARED_PID=$(service_pid "com.melodysync.tunnel")
    ...
    systemctl --user start melodysync-chat.service
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        systemctl --user enable melodysync-tunnel.service 2>/dev/null || true
        systemctl --user start melodysync-tunnel.service
    fi
    sleep 3
    ...
    if [[ "$USE_CLOUDFLARE" == true ]]; then
        if systemctl --user is-active --quiet melodysync-tunnel.service; then
...
if [[ "$USE_CLOUDFLARE" == true ]]; then
    echo -e "${GREEN}✓ MelodySync is now accessible remotely!${NC}"
    echo ""
