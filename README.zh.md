...
```text
我想在这台机器上配置 MelodySync，这样我就能从不同设备把重复数字工作交给 AI，并让它在真实机器上完成自动化。
...
请把 `https://raw.githubusercontent.com/YOUR_GITHUB_USERNAME/melody-sync/main/docs/setup.md` 当作配置契约和唯一真相来源。
不要假设这个仓库已经提前 clone 到本地。如果 `~/code/melody-sync` 还不存在，请你先读取那份契约，再自行 clone `https://github.com/YOUR_GITHUB_USERNAME/melody-sync.git`，然后继续完成安装。
后续流程都留在这个对话里。
...
- 首屏不能只是一个空的 session list。新用户需要一个默认的 `Welcome App`：先用很短的话说明 MelodySync 能做什么，再收集用户的角色背景和重复工作痛点，把他们引导到第一个具体 automation 上。
...
```text
我想在这台机器上配置 MelodySync，这样我就能从不同设备控制 AI worker，并把长时间运行的 AI 工作组织起来。
...
melodysync restart [service]    重启：chat | tunnel | all
...
| `~/.config/melody-sync/auth-sessions.json` | Owner / visitor 登录会话 |
...
| `~/.config/melody-sync/apps.json` | App 模板定义 |
| `~/.config/melody-sync/shared-snapshots/` | 不可变的只读会话分享快照 |
...
| `~/.local/share/melody-sync/logs/cloudflared.log` | Tunnel 标准输出 **(Linux)** |

## 安全

- **Cloudflare 模式**：通过 Cloudflare 提供 HTTPS（边缘 TLS，机器侧仍是本地 HTTP）；服务只绑定 `127.0.0.1`
- **Tailscale 模式**：流量由 Tailscale 的 WireGuard mesh 加密；服务绑定 `0.0.0.0`（所有接口），因此端口也可从局域网/公网访问——在不可信网络中，建议配置防火墙将 `7760` 端口限制为 Tailscale 子网（如 `100.64.0.0/10`）
