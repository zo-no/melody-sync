...
assert.equal(candidateOnlyElements.get('questTrackerNext').hidden, true, 'mainline tracker should stop using a secondary hint lane for candidate counts');
assert.equal(candidateOnlyElements.get('questTrackerDetailToggle').hidden, true, 'candidate-only mainline tasks should keep branch suggestions in the task map instead of the tracker detail');
...
const candidateOpenBoard = findFirstByClass(candidateOpenElements.get('questTaskList'), 'quest-task-flow-shell');
const trackerCandidateToOpen = findAllByClass(candidateOpenBoard, 'quest-task-flow-node')
  .find((node) => findFirstByClass(node, 'quest-task-flow-node-title')?.textContent === '改成视觉风格线');
findAllByTagName(trackerCandidateToOpen, 'button')
  .find((entry) => entry?.textContent === '开启')
  ?.click();
await flushAsync();
assert.equal(
  ...
  true,
  'clicking a task-map candidate suggestion should open a real branch through the existing branch-creation endpoint',
);
assert.deepEqual(
  candidateOpenAttachCalls.map((entry) => entry.id),
  ['session-main-candidate-branch'],
  'opening a task-map candidate suggestion should attach the newly created branch session into the main workspace flow',
);
...
assert.equal(
  ...
  true,
  'memory candidate review should no longer expand the tracker into an approval queue',
);
assert.equal(
  memoryReviewElements.get('questTrackerDetailToggle').hidden,
  true,
  'memory candidate review should stay outside the simplified tracker surface',
);
assert.equal(
  ...
  false,
  'simplifying the tracker should avoid auto-triggering memory review actions from the task bar',
);
