...
assert.ok(/--quest-task-flow-running-accent:\s*var\(--notice\);[\s\S]*--quest-task-flow-node-status-accent:\s*var\(--quest-task-flow-running-accent\);/i.test(sources.workbench), 'running flow nodes should derive their accent from the shared notice token');
assert.ok(/--quest-task-flow-completed-accent:\s*var\(--success\);[\s\S]*--quest-task-flow-node-status-accent:\s*var\(--quest-task-flow-completed-accent\);/i.test(sources.workbench), 'completed flow nodes should derive their accent from the shared success token');
