...
- Single-session delegation already exists through `POST /api/sessions/:sessionId/delegate` and `melodysync session-spawn`.
...
- Judge the next push by whether the owner can operate MelodySync cleanly through session-first surfaces after the current board is gone.
...
- The owner can operate MelodySync cleanly after the shipped `Board` surface is removed.
