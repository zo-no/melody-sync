...
- `builtin.session-naming`
  - 文件：`backend/hooks/session-naming-hook.mjs`
  - 触发：`run.completed`
...
- `backend/workbench/node-task-card.mjs`
...
- `backend/workbench/node-task-card-sync.mjs`
...
- `backend/workbench/task-map-graph-service.mjs`
...
- `node-task-card.js`
  ...
  - 当前和 backend `backend/workbench/node-task-card.mjs` 保持同一条优先级规则：`plan/manual/hook` 节点可以覆盖默认 projection 节点的标量绑定
...
- backend workbench state 里的 `nodes`
  - 是持久化工作台对象，来源于 `backend/workbench/index.mjs`
