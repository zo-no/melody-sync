(function () {
  ...
  function loadScript(src, { optional = false } = {}) {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        ...
        if (optional) {
          console.warn("[chat] Optional frontend asset missing:", src);
          resolve(false);
          return;
        }
        reject(error);
  ...
  (async () => {
    ...
    for (const path of [...requiredAssetPaths, ...optionalAssetPaths]) {
    ...
    for (const path of optionalAssetPaths) {
      await loadScript(buildVersionedAssetPath(path, assetVersion), { optional: true });
    }
  })().catch((error) => {
