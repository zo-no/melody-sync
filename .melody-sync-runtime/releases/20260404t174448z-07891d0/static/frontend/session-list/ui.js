...
function renderPersistentSessionDock(persistentSessionsByGroup) {
  ...
  container.innerHTML = "";
  container.className = "session-list-persistent-dock";

  if (hasLongTerm) {
    container.appendChild(renderPersistentDockSection("group:long-term", persistentSessionsByGroup["group:long-term"]));
  }
  if (hasQuickActions) {
    container.appendChild(renderPersistentDockSection("group:quick-actions", persistentSessionsByGroup["group:quick-actions"]));
  }
}
...
function renderSessionList() {
  ...
  for (const session of visibleSessions) {
    ...
    if (groupInfo.key === "group:long-term" || groupInfo.key === "group:quick-actions") {
      if (!Array.isArray(persistentSessionsByGroup[groupInfo.key])) {
        persistentSessionsByGroup[groupInfo.key] = [];
      }
      persistentSessionsByGroup[groupInfo.key].push(session);
      continue;
    }
