...
export async function promoteSessionToPersistent(id, payload = {}) {
  ...
  const result = await mutateSessionMeta(id, (draft) => {
    let changed = false;
    const persistentGroup = getPersistentSessionGroup(nextPersistent.kind);
    if (persistentGroup && draft.group !== persistentGroup) {
      draft.group = persistentGroup;
      changed = true;
    }
    if (JSON.stringify(normalizeSessionPersistent(draft.persistent || null, {
      defaultDigest,
      defaultRuntime: draft,
      defaultTimezone: Intl.DateTimeFormat().resolvedOptions().timeZone || '',
    })) !== JSON.stringify(nextPersistent)) {
