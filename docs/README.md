...
7. `output-panel-data-contract.md` — decision-first data contract for output measurement, blocker capture, and recommendation calibration
8. `agent-filesystem-architecture.md` — current repo/config/memory filesystem split for agent-facing storage and runtime truth
9. `hooks-node-architecture.md` — target hooks + node architecture for configurable lifecycle orchestration and AI-friendly extension
10. `../notes/current/core-domain-contract.md` — current domain/refactor baseline
11. `../notes/current/session-first-workflow-surfaces.md` — current workflow-organization contract for session list / grouping / task-like views
12. `../notes/current/product-surface-lifecycle.md` — current keep/iterate/retire rule for shipped product surfaces
13. `../notes/current/session-run-closure-requirements.md` — next-stage task closure direction after the current refactor line
14. `structural-cleanup-plan.md` — staged cleanup contract for removing residue without breaking the main session flow
15. `setup.md` / `external-message-protocol.md` / other focused guides as needed
...
- `output-panel-data-contract.md` — decision-first output-panel data contract: event taxonomy, blocker capture, artifact truth, and recommendation feedback
