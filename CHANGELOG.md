...
- 点击「立即执行」（或「触发AI快捷按钮」）后，transcript 区域现在会立即显示自动发出的消息，不再需要等待 WS 推送才刷新。
- 持久任务自动发送的消息格式更新：标题行改为 `[触发执行] 任务名` / `[定时执行] 任务名` / `[单次执行] 任务名`，去掉原来单独一行的「触发方式」字段，任务名内联到标题中更简洁。
- 设置执行方式弹窗默认选中「定时任务」而非「AI快捷按钮」。
