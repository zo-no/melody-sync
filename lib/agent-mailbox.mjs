...
import { CHAT_PORT, MELODYSYNC_APP_ROOT } from './config.mjs';
import {
...
function sanitizeGuestInstanceName(value) {
...
const DEFAULT_ROOT_DIR = join(MELODYSYNC_APP_ROOT, 'email');
...
const DEFAULT_MAILBOX_DESCRIPTION = 'Agent-facing mailbox identity for MelodySync collaboration.';
...
function normalizeMailboxIdentity(value) {
  ...
  return {
    ...
    description: trimString(value.description) || DEFAULT_MAILBOX_DESCRIPTION,
...
function normalizeOutboundConfig(value = {}) {
  ...
  return {
    ...
    provider: provider === 'apple_mail' ? 'apple_mail' : DEFAULT_OUTBOUND_CONFIG.provider,
...
function saveMailboxIdentity(rootDir = DEFAULT_ROOT_DIR, identity = {}) {
  const current = loadIdentity(rootDir) || {};
  const createdAt = trimString(current.createdAt) || nowIso();
  const updatedAt = nowIso();
  const normalized = normalizeMailboxIdentity({
    ...current,
    ...(identity || {}),
    createdAt,
    updatedAt,
  ...
  writeJson(mailboxPaths(rootDir).identityFile, {
    ...normalized,
    createdAt,
    updatedAt,
    status: trimString(identity?.status) || trimString(current.status) || 'local_intake_ready_public_dns_pending',
  });
  return loadIdentity(rootDir);
}
...
function initializeMailbox({ rootDir = DEFAULT_ROOT_DIR, name, localPart, domain, description, allowEmails = [], allowDomains = [], instanceAddressMode }) {
  ...
  const identity = {
    ...
    description: description || DEFAULT_MAILBOX_DESCRIPTION,
