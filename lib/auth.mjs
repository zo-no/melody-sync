...
function loadAuth() {
  try {
    ...
    // Return null if auth.json doesn't exist yet (e.g. during set-password init)
    // The server startup code is responsible for exiting if auth is missing.
...
export function getAuthSession(req) {
  return normalizeAuthSession(getValidSession(getSessionToken(req)));
}
