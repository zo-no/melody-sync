...
Options:
  ...
  --app-root <path>              Override the MelodySync app root to inspect
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      case '--apply':
        ...
        break;
      case '--app-root':
        options.appRoot = trimString(argv[index + 1]);
        index += 1;
        break;
      case '--api-log-days':
...
function resolveCommandPaths(appRoot) {
  if (appRoot) {
    const paths = buildMelodySyncPaths(appRoot);
    return {
      basePath: paths.root,
      apiRequestLogsDir: paths.apiRequestLogsDir,
      chatRunsDir: paths.chatRunsDir,
      codexManagedHomeDir: paths.codexManagedHomeDir,
    };
  }
  return {
    basePath: MELODYSYNC_APP_ROOT || CONFIG_DIR,
...
export async function runStorageMaintenanceCommand(argv = [], io = {}) {
  ...
  const paths = resolveCommandPaths(options.appRoot);
