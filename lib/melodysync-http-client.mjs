...
export function createMelodySyncHttpClient(options = {}) {
  const runtime = {
    baseUrl: normalizeBaseUrl(options.baseUrl || process.env.MELODYSYNC_CHAT_BASE_URL || DEFAULT_CHAT_BASE_URL),
  ...
  async function request(path, options = {}) {
    ...
    if (!normalizedPath.startsWith('/')) {
      throw new Error(`MelodySync API path must start with "/": ${path}`);
    }
