...
export const RELEASE_RUNTIME_DIR = join(SOURCE_PROJECT_ROOT, '.melody-sync-runtime');
...
export const RELEASE_METADATA_FILENAME = '.melody-sync-release.json';
...
export function shouldUseActiveRelease() {
  if (isTruthyEnv(process.env.MELODYSYNC_DISABLE_ACTIVE_RELEASE)) {
    return false;
  }
  if (isTruthyEnv(process.env.MELODYSYNC_ENABLE_ACTIVE_RELEASE)) {
    return true;
  }
  const configuredPort = trimString(process.env.CHAT_PORT);
  return !configuredPort || configuredPort === '7760';
}
...
export function resolveActiveReleaseManifestPath() {
  const explicitPath = trimString(process.env.MELODYSYNC_ACTIVE_RELEASE_FILE);
  return explicitPath ? resolve(explicitPath) : ACTIVE_RELEASE_FILE;
}

export async function readActiveReleaseManifest() {
  const explicitSnapshotRoot = trimString(process.env.MELODYSYNC_ACTIVE_RELEASE_ROOT);
  if (explicitSnapshotRoot) {
    return validateSnapshotRoot(explicitSnapshotRoot, {
      releaseId: trimString(process.env.MELODYSYNC_ACTIVE_RELEASE_ID),
      sourceRoot: trimString(process.env.MELODYSYNC_SOURCE_PROJECT_ROOT || SOURCE_PROJECT_ROOT),
    });
  }
