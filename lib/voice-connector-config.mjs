...
export const LEGACY_DEFAULT_SESSION_SYSTEM_PROMPT = [
  'You are replying through a local wake-word voice connector powered by MelodySync.',
...
function normalizeTtsConfig(value) {
  ...
  const command = trimString(normalized.command);
  const requestedMode = trimString(normalized.mode).toLowerCase();
  const legacyMode = requestedMode === 'xfyun'
    ? (command ? 'command' : 'disabled')
    : requestedMode;
  const defaultMode = command ? 'command' : (process.platform === 'darwin' ? 'say' : 'disabled');
  const mode = legacyMode || defaultMode;
  if (!['command', 'say', 'xfyun', 'disabled', 'off'].includes(mode)) {
  ...
  return {
    ...
    mode: mode === 'off' || mode === 'xfyun' ? 'disabled' : mode,
    command,
