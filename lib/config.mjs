...
export const DEFAULT_OBSIDIAN_VAULT_DIR = '/Users/kual/Desktop/diary/diary';
export const MELODYSYNC_APP_ROOT_SEGMENTS = Object.freeze(['.melodysync']);
export const MELODYSYNC_APP_ROOT_NAME = MELODYSYNC_APP_ROOT_SEGMENTS[MELODYSYNC_APP_ROOT_SEGMENTS.length - 1];
...
function resolveMelodySyncAppRoot(vaultPath) {
  ...
  const preferredWorkspaceDir = join(normalizedVaultPath, MELODYSYNC_AGENT_WORKSPACE_DIRNAME);
  const containerRoot = existsSync(preferredWorkspaceDir) ? preferredWorkspaceDir : normalizedVaultPath;
  return join(containerRoot, ...MELODYSYNC_APP_ROOT_SEGMENTS);
}
...
function resolveMelodySyncDefaultAgentsPath(vaultPath) {
  ...
  if (!appRoot) return '';
  return join(dirname(appRoot), MELODYSYNC_AGENTS_FILENAME);
}
...
function normalizeConfiguredAgentsPath(value, storageRootPath) {
  ...
  const legacyVisibleAgentsPath = storageRootPath
    ...
    : '';
  return resolvedPath === legacyHiddenAgentsPath || resolvedPath === legacyVisibleAgentsPath
    ? ''
    : resolvedPath;
}
...
function buildMelodySyncPaths(appRoot, options = {}) {
  ...
  const agentsFile = resolveOverridePath(options.agentsFile) || join(dirname(root), MELODYSYNC_AGENTS_FILENAME);
...
这是 MelodySync 在本地数据目录中的用户可编辑 agent 说明文件。

## 默认角色

- 这个文件负责告诉 agent：如何理解当前本地知识库/工作区，以及 MelodySync 在其中扮演什么角色。
- MelodySync 的程序数据统一保存在 \`.melodysync/\` 中。
- 如果当前本地数据根路径本身就是一个 Obsidian vault，那么这个文件可以进一步告诉 agent 如何管理整个 vault。

## 推荐读取顺序

1. 先读本文件，确认当前知识库/工作区的管理边界
2. 再读 \`.melodysync/memory/bootstrap.md\`
3. 按需读 \`.melodysync/memory/projects.md\`、\`.melodysync/memory/skills.md\`
4. 任务明确后再读 \`.melodysync/memory/tasks/\` 和当前 vault 中相关项目文件

## MelodySync 程序数据目录

- 应用目录：\`.melodysync/\`
- 配置：\`.melodysync/config/\`
- 记忆：\`.melodysync/memory/\`
- 会话：\`.melodysync/sessions/\`
- Hooks：\`.melodysync/hooks/\`
- Workbench：\`.melodysync/workbench/\`
- 日志：\`.melodysync/logs/\`

## 自定义 Hooks

- 自定义 hook 定义文件：\`.melodysync/hooks/custom-hooks.json\`
- 这里可以放本地 shell 脚本 hook，例如启动时打开 Obsidian 或执行同步脚本。

## 如果当前目录是 Obsidian vault

- 顶层分区由人类知识组织决定，agent 需要遵守现有目录语义。
- MelodySync 不应该把业务数据散落到各个知识目录里，程序状态统一放在 \`.melodysync/\`。
- 需要管理整个 vault 时，由本文件明确各个顶层目录的用途、可写边界和默认读取顺序。
...
这个目录是 MelodySync 在当前本地数据根路径下的统一应用根目录。
...
const defaultUserMemoryDir = join(homedir(), '.melody-sync', 'memory');
...
const inferredUserConfigDir = existsSync(defaultUserConfigDir)
  ...
  : legacyUserConfigDir;
const inferredUserMemoryDir = existsSync(defaultUserMemoryDir)
...
export const USE_OBSIDIAN_VAULT_STORAGE = !explicitConfigOverride && !explicitMemoryOverride && !INSTANCE_ROOT && !!OBSIDIAN_VAULT_DIR;
export const MELODYSYNC_APP_ROOT = USE_OBSIDIAN_VAULT_STORAGE
  ? resolveMelodySyncAppRoot(OBSIDIAN_VAULT_DIR)
  : '';
export const MELODYSYNC_LEGACY_VAULT_ROOT = USE_OBSIDIAN_VAULT_STORAGE
  ? resolveMelodySyncLegacyVaultRoot(OBSIDIAN_VAULT_DIR)
  : '';
export const MELODYSYNC_LEGACY_VISIBLE_VAULT_ROOT = USE_OBSIDIAN_VAULT_STORAGE
  ? resolveMelodySyncLegacyVisibleVaultRoot(OBSIDIAN_VAULT_DIR)
  : '';
const defaultVaultAgentsFile = USE_OBSIDIAN_VAULT_STORAGE
  ? resolveMelodySyncDefaultAgentsPath(OBSIDIAN_VAULT_DIR)
  : '';
const configuredVaultAgentsFile = USE_OBSIDIAN_VAULT_STORAGE
  ? (bootstrapGeneralSettings.agentsPath || defaultVaultAgentsFile)
  : '';
const vaultPaths = USE_OBSIDIAN_VAULT_STORAGE
  ? buildMelodySyncPaths(MELODYSYNC_APP_ROOT, { agentsFile: configuredVaultAgentsFile })
  : null;
export const CONFIG_DIR = USE_OBSIDIAN_VAULT_STORAGE
  ? vaultPaths.configDir
  : (explicitConfigOverride || (INSTANCE_ROOT ? join(INSTANCE_ROOT, 'config') : inferredUserConfigDir));
export const MELODYSYNC_DEFAULT_AGENTS_FILE = USE_OBSIDIAN_VAULT_STORAGE
  ? defaultVaultAgentsFile
  : join(CONFIG_DIR, MELODYSYNC_AGENTS_FILENAME);
export const MELODYSYNC_CONFIGURED_AGENTS_FILE = USE_OBSIDIAN_VAULT_STORAGE
  ? configuredVaultAgentsFile
  : join(CONFIG_DIR, MELODYSYNC_AGENTS_FILENAME);

export const MEMORY_DIR = explicitMemoryOverride
  || (INSTANCE_ROOT ? join(INSTANCE_ROOT, 'memory') : (USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.memoryDir : inferredUserMemoryDir));

await mkdir(CONFIG_DIR, { recursive: true });

if (USE_OBSIDIAN_VAULT_STORAGE && vaultPaths) {
  await ensureMelodySyncVaultScaffold(vaultPaths);
  await migrateLegacyStorageToVault(vaultPaths, {
    useVaultStorage: USE_OBSIDIAN_VAULT_STORAGE,
...
export const AUTH_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.authFile : join(CONFIG_DIR, 'auth.json');
export const TOOLS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.toolsFile : join(CONFIG_DIR, 'tools.json');
export const AUTH_SESSIONS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.authSessionsFile : join(CONFIG_DIR, 'auth-sessions.json');
export const CHAT_SESSIONS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatSessionsFile : join(CONFIG_DIR, 'chat-sessions.json');
export const CHAT_TRIGGERS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatTriggersFile : join(CONFIG_DIR, 'chat-triggers.json');
export const CHAT_HISTORY_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatHistoryDir : join(CONFIG_DIR, 'chat-history');
export const CHAT_RUNS_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatRunsDir : join(CONFIG_DIR, 'chat-runs');
export const CHAT_IMAGES_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatImagesDir : join(CONFIG_DIR, 'images');
export const CHAT_FILE_ASSETS_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatFileAssetsDir : join(CONFIG_DIR, 'file-assets');
export const CHAT_FILE_ASSET_CACHE_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.chatFileAssetCacheDir : join(CONFIG_DIR, 'file-assets-cache');
export const API_REQUEST_LOGS_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.apiRequestLogsDir : join(CONFIG_DIR, 'api-logs');
export const VAPID_KEYS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.vapidKeysFile : join(CONFIG_DIR, 'vapid-keys.json');
export const PUSH_SUBSCRIPTIONS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.pushSubscriptionsFile : join(CONFIG_DIR, 'push-subscriptions.json');
export const APPS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.appsFile : join(CONFIG_DIR, 'apps.json');
export const USERS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.usersFile : join(CONFIG_DIR, 'users.json');
export const HOOKS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.hooksFile : join(CONFIG_DIR, 'hooks.json');
export const CUSTOM_HOOKS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.customHooksFile : join(CONFIG_DIR, 'custom-hooks.json');
export const GENERAL_SETTINGS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.generalSettingsFile : join(CONFIG_DIR, 'general-settings.json');
export const WORKBENCH_NODE_SETTINGS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchNodeSettingsFile : join(CONFIG_DIR, 'workbench-node-settings.json');
export const WORKBENCH_CAPTURE_ITEMS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchCaptureItemsFile : join(CONFIG_DIR, 'workbench-capture-items.json');
export const WORKBENCH_PROJECTS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchProjectsFile : join(CONFIG_DIR, 'workbench-projects.json');
export const WORKBENCH_NODES_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchNodesFile : join(CONFIG_DIR, 'workbench-nodes.json');
export const WORKBENCH_BRANCH_CONTEXTS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchBranchContextsFile : join(CONFIG_DIR, 'workbench-branch-contexts.json');
export const WORKBENCH_TASK_MAP_PLANS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchTaskMapPlansFile : join(CONFIG_DIR, 'workbench-task-map-plans.json');
export const WORKBENCH_SKILLS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchSkillsFile : join(CONFIG_DIR, 'workbench-skills.json');
export const WORKBENCH_SUMMARIES_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.workbenchSummariesFile : join(CONFIG_DIR, 'workbench-summaries.json');
export const UI_RUNTIME_SELECTION_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.uiRuntimeSelectionFile : join(CONFIG_DIR, 'ui-runtime-selection.json');
export const CODEX_MANAGED_HOME_DIR = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.codexManagedHomeDir : join(CONFIG_DIR, 'provider-runtime-homes', 'codex');
export const MELODYSYNC_AGENTS_FILE = USE_OBSIDIAN_VAULT_STORAGE ? vaultPaths.agentsFile : join(CONFIG_DIR, MELODYSYNC_AGENTS_FILENAME);
