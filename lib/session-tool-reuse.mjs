...
const BOOTSTRAP_TOKENS = new Set([
  ...
  'agents', 'agents.md', 'agent-profile', 'agent-profile.md', 'context-digest', 'context-digest.md',
  'worklog', 'system', 'system.md', 'self-review.md',
]);
...
export function generateSessionToolReuseSidecar({
  ...
  const effectiveConfigDir = configDir || join(HOME, '.config', 'melody-sync');
  const effectiveOutputDir = outputDir || join(HOME, '.melodysync', 'reports', 'session-tool-reuse');
