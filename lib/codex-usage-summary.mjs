...
function isNoiseUserMessage(value) {
  ...
  return [
    ...
    /^You are an AI agent operating on this computer via MelodySync\./i,
...
export function getCodexUsageRoots(options = {}) {
  ...
  if (source === 'all' || source === 'managed') {
    roots.push({
      id: 'managed',
      label: 'managed (MelodySync)',
      path: join(homeDir, '.config', 'melody-sync', 'provider-runtime-homes', 'codex', 'sessions'),
    });
  }
