...
import {
  ...
  loadAssistantReply,
  trimString,
} from './melodysync-http-client.mjs';

function printHelp(stdout = process.stdout) {
  stdout.write(`Usage:\n  melodysync api <METHOD> <PATH> [options]\n\nExamples:\n  melodysync api GET /api/tools\n  melodysync api POST /api/sessions --body '{"folder":"~/code/melody-sync","tool":"micro-agent","name":"scratch"}'\n  melodysync api POST /api/sessions/<session-id>/messages --body '{"text":"hello"}' --wait-run\n\nOptions:\n  --body <json>            JSON request body; use - to read from stdin\n  --body-file <path>       Read JSON request body from a file\n  --wait-run               If the response contains run.id, wait for terminal state\n  --timeout-ms <ms>        Wait timeout for --wait-run (default: 600000)\n  --base-url <url>         MelodySync base URL (default: $MELODYSYNC_CHAT_BASE_URL or local 7760)\n  --help                   Show this help\n`);
}
...
function parseArgs(argv = []) {
  const options = {
    ...
    baseUrl: trimString(process.env.MELODYSYNC_CHAT_BASE_URL || DEFAULT_CHAT_BASE_URL),
    help: false,
  };
...
export async function runMelodySyncApiCommand(argv = [], io = {}) {
  ...
  if (!options.method || !options.path) {
    throw new Error('Usage: melodysync api <METHOD> <PATH> [options]');
  }

  const client = createMelodySyncHttpClient({ baseUrl: options.baseUrl });
  ...
  if (!result.response.ok) {
    throw new Error(result.json?.error || result.text || `MelodySync request failed (${result.response.status})`);
  }
