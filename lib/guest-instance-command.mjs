...
const OWNER_CONFIG_DIR = join(HOME_DIR, '.config', 'melody-sync');
const DEFAULT_GUEST_INSTANCES_ROOT = join(HOME_DIR, '.melodysync', 'instances');
...
function printHelp(stdout = process.stdout) {
  stdout.write(`Usage:\n  melodysync guest-instance <command> [options]\n\nCommands:\n  create <name>            Create and start an isolated instance\n  list                     List isolated instances created by this tool\n  show <name>              Show one isolated instance and its current access URL\n\nCreate options:\n  --port <port>            Explicit port (default: next free port from ${DEFAULT_GUEST_INSTANCE_START_PORT})\n  --instance-root <path>   Instance root (default: ~/.melodysync/instances/<name>)\n  --session-expiry-days <days>  Cookie lifetime in days (default: ${DEFAULT_GUEST_SESSION_EXPIRY_DAYS})\n  --username <name>        Optional password-login username (default: owner when --password is set)\n  --password <value>       Optional password-login password\n\nGeneral options:\n  --json                   Print machine-readable JSON\n  --help                   Show this help\n\nExamples:\n  melodysync guest-instance create trial4\n  melodysync guest-instance create demo --json\n  melodysync guest-instance list\n  melodysync guest-instance show trial4\n`);
}
...
function collectReservedPorts(registry) {
...
function chooseGuestPort(options, registry) {
  const reservedPorts = collectReservedPorts(registry);
...
function fetchBuildInfoWithCurl(baseUrl, timeoutMs) {
  const statusMarker = '__MELODYSYNC_HTTP_STATUS__:';
  try {
...
function formatGuestInstanceList(records = []) {
  if (records.length === 0) return 'No isolated instances found.';
...
async function createGuestInstance(options) {
  ...
  if (!name) {
    throw new Error('create requires an instance name');
  }
  ...
  const port = chooseGuestPort(options, registry);
  ...
  const plistContent = buildLaunchAgentPlist({
    ...
    environmentVariables: {
      ...
      MELODYSYNC_INSTANCE_ROOT: instanceRoot,
...
async function showGuestInstance(name) {
  const normalizedName = sanitizeGuestInstanceName(name);
  if (!normalizedName) {
    throw new Error('show requires an instance name');
  }
  const registry = await loadGuestRegistry();
