...
function splitCommandWithArgs(command) {
  ...
  if (!raw) return { command: '', commandArgs: [] };
...
function normalizeSimpleToolRecord(tool) {
  ...
  const commandSpec = splitCommandWithArgs(tool.command);
  ...
  const commandArgs = [
    ...normalizeCommandArgs(commandSpec.commandArgs),
    ...normalizeCommandArgs(tool.commandArgs),
  ];
...
export async function saveSimpleToolAsync({
  name,
  command,
  commandArgs = [],
  runtimeFamily,
  ...
  const record = {
    ...
    ...(normalizedCommandArgs.length > 0 ? { commandArgs: normalizedCommandArgs } : {}),
