...
### 3. 长期任务（recurring_task）执行完成时 — 收集反馈

长期任务每次触发执行后，根据任务类型决定是否收集用户反馈：

**判断标准：**
- `runPrompt` 中包含"请确认用户是否完成"或类似指令 → 在回复末尾询问用户，并将结果写入 `knownConclusions`
- `runPrompt` 中没有此类指令（纯 AI 执行任务，如数据收集）→ 不询问，直接产出结果

**反馈写入格式（当需要收集反馈时）：**

在 `<task_card>` 的 `knownConclusions` 中追加：
```
"YYYY-MM-DD: 完成 ✓"  或  "YYYY-MM-DD: 未完成（原因）"
```

最多保留最近 4 条记录（旧记录自动被新记录覆盖）。这些记录会在下次触发时作为"核心记录"传入上下文，帮助 AI 了解执行历史并调整计划。

**示例（每周资产记录任务）：**
- AI 触发 → 提醒用户记录本周资产 → 询问"本周资产记录完成了吗？"
- 用户回复"完成了" → AI 写入 `knownConclusions: ["2026-04-14: 完成 ✓"]`
- 下次触发时，AI 看到历史记录，可据此调整提醒方式

**示例（每日行情收集任务）：**
- AI 触发 → 自动收集数据 → 写入知识库 → 不询问用户（AI 自己完成了）
